...
Raphael.el.addClassAttribute = function(attrStr, noUpdate) {
  ...
  if (this.classAttributes.contains(attrStr)) {
...
Raphael.el.removeClassAttribute = function(attrStr, noUpdate) {
  if (! this.classAttributes || ! this.classAttributes.contains(attrStr)) {
    return; // nothing to do
  }
...
Raphael.el.containsClassAttribute = function (attrStr) {
  return this.classAttributes.contains(attrStr);
};
...
R_fn.computeArrowPath = function(from, to, baseWidth) {
...
R_fn.arrow = function (from, to, baseWidth) {
  return this.path(this.computeArrowPath(from,to,baseWidth));
}
//}; // R_fn.eg
...
R_fn.eg.createTextFieldAtXY = function (x,y, textArg, attrsOrNil) {
  ...
  var attrs = this.defaultTextFieldAttributes();
  if (attrsOrNil) {
  ...
  if (textArg && textArg !== " ") {
    ...
    if (textArg.length <= limit) {
      ...
      var minWrapPos = textArg.length / 2;
      // if no wrapPos found (-1) after minWrapPos, use minWrapPos
      var wrapPos = Math.max(textArg.indexOf(" ", minWrapPos), minWrapPos);
      text = textArg.substring(0, wrapPos)
        + "\n" + textArg.substring(wrapPos);
    }
    // does not work:
...
R_fn.eg.createTextFieldAt = function (pos, text, attrsOrNil) {
  return this.createTextFieldAtXY(pos.x,pos.y, text, attrsOrNil);
};
R_fn.eg.createTextField = function (text, attrsOrNil) {
  return this.createTextFieldAtXY(0,0, text, attrsOrNil);
};
...
R_fn.eg.createTextButtonAtXY = function (x,y, text, attrsOrNil) {
  var textField = this.createTextFieldAtXY(x,y, text, attrsOrNil);
  return this.createButtonFrom(textField);
};
R_fn.eg.createTextButtonAt = function (pos, text, attrsOrNil) {
  return this.createTextButtonAtXY(pos.x, pos.y, text, attrsOrNil);
};
R_fn.eg.createTextButton = function (text, attrsOrNil) {
  return this.createTextButtonAtXY(0, 0, text, attrsOrNil);
};
/*
...
R_fn.eg.createOnOffButtonFrom = function (offButton, onButton,
					  alignmentFlag) {
  var fneg = R_fn.eg;
  var onOffButton = this.createWidget();
...
R_fn.eg.createOnOffTextButtonAt = function (pos, textOff, textOn, textAttributesOff, textAttributesOn,
  ...
  var textFieldOff = this.createTextFieldAt(eg.Point.zero, textOff, textAttributesOff, bgAttributesOff, minDimensions);
  var textFieldOn = this.createTextFieldAt(eg.Point.zero, textOn, textAttributesOn, bgAttributesOn, minDimensions);
  var dimsToFitBoth = textFieldOff.extent().max(textFieldOn.extent());
  textFieldOff.remove();
  textFieldOn.remove();
  var offButton = this.createTextButtonAt(pos, textOff, textAttributesOff, bgAttributesOff, dimsToFitBoth);
  var onButton = this.createTextButtonAt(pos, textOn, textAttributesOn, bgAttributesOn, dimsToFitBoth);
  return this.createOnOffButtonFrom(offButton, onButton, true);
};
...
R_fn.eg.createOnOffTextButton = function (textOff, textOn, textAttributesOff, textAttributesOn,
  ...
  return this.createOnOffTextButtonAt(eg.Point.zero, textOff, textOnInner, textAttributesOff, textAttributesOn,
...
R_fn.eg.createRadioButtons = function (onOffButtons, onIndexOrNil) {
  var radioButtons = this.createWidget();
...
R_fn.eg.createMilestonesWidget = function (pos, width, milestoneExtent, attributes) {
  var self = this;
  var attrs = attributes || this.defaultMilestonesWidgetAttributes();

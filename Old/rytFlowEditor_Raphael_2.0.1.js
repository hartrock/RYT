...
//o_      if ((e.altKey || target.raphael && target.raphael.containsClassAttribute('connectArea'))
      if ((e.altKey || target.raphael && self.r.getById(target.raphaelid).containsClassAttribute('connectArea'))
    ...
    canvas.onmousemove = function (e) {
      ...
      if (! self.state.dragPos) {
        if (self.state.mousedownPos && self.state.selected.size()) { // startDrag
...
*/
  ...
  protoFE.moveWidgets = function (widgetPosArr, sender, argObj) {
    ...
    for (i = 0; i < len; ++i) {
      ...
      if (doAnimation) {
        ...
        //eg.log("offP:", offP);
        w.stop();
        w.animate(//{translation:""+offP.x+","+offP.y},
                  {transform:"...t"+offP.x+","+offP.y},
                  animationTime, // this keeps text field offset ..
                  //"linear",
  ...
  protoFE.roll = function (conn, recursivelyFlag, reversedFlag) {
    ...
    ++this.busyCount;
/*
    var linePath = conn.line.attrs.path;
    var path, colorStr;
    if (reversedFlag) {
      path = this.reversePath(linePath);
      colorStr = "#f00";
    } else {
      path = linePath;
      colorStr = "#0f0";
    }
    var pathLength = conn.line.getTotalLength();
    var marble = self.makeMarble(colorStr);
    marble.moveTo(path[0][1] -marbleRadius, path[0][2] -marbleRadius);
    marble.toFront().show();
    marble.animateAlong(
      path, pathLength / 200 * this.animationTime, false,
      function(){
        marble.remove();
        if (recursivelyFlag) {
    ...
    */
    ...
    var marble = self.makeMarble(reversedFlag ? "#f00" : "#0f0");
    var customFuncName = "along_" + this.busyCount;
    this.r.customAttributes[customFuncName] = function (v) {
      var point = path.getPointAtLength(v * pathLength);
      return {
        transform: "t" + [point.x + marbleRadius, point.y + marbleRadius]// + "r" + point.alpha
      };
    };
    var startAttr = {}; startAttr[customFuncName] = reversedFlag ? 1 : 0;
    marble.attr(startAttr);  // has to be after customAttributes func
    marble.toFront().show();
    function run() {
      var stopAttr = {};
      stopAttr[customFuncName] = reversedFlag ? 0 : 1;;
      marble.animate(
        stopAttr, self.animationTime,
        function () {
          marble.remove();
          delete self.r.customAttributes[customFuncName];
          if (recursivelyFlag) {
            self.rollFor(conn.to, true, reversedFlag);
          }
          --self.busyCount;
